...
| reminderMormaiiSmartwatch | Bool | Mormaii Smartwatch |
...
| reminderWeibo | Bool | 国内版微博 |
| reminderTaobao | Bool | 淘宝 |
| reminderDingding | Bool | 钉钉 |
| reminderAlipay | Bool | 支付宝 |
| reminderToutiao | Bool | 今日头条 |
| reminderDouyin | Bool | 抖音 |
| reminderTmall | Bool | 天猫 |
| reminderJd | Bool | 京东 |
| reminderPinduoduo | Bool | 拼多多 |
| reminderBaidu | Bool | 百度 |
| reminderMeituan | Bool | 美团 |
| reminderEleme | Bool | 饿了么 |
| reminderGoogleMessages | Bool | 通知支持 Google Messages，type 0x7B（table62 BIT_3） |
| reminderAppleCalendar | Bool | 通知支持 Apple Calendar，type 0x7C（table62 BIT_4） |
| reminderAppleMail | Bool | 通知支持 Apple Mail，type 0x7D（table62 BIT_5） |
| reminderGoogleCalendar | Bool | 通知支持 Google Calendar，type 0x52（table62 BIT_6） |
| reminderHealthU | Bool | 支持 Health-U 通知，type 0x7F（table62 BIT_7） |
| reminderZalo | Bool | 支持 Zalo 通知（table66） |
| reminderDidiTaxi | Bool | 支持滴滴打车通知（table67） |
| reminderZoomWorkplace | Bool | 通知支持 Zoom Workplace，type 0x82（table71 BIT_5） |
| reminderBuzud | Bool | 通知支持 BUZUD，type 0x83（table72 BIT_0） |
...
| getBtVersion | Bool | 获得固件三级版本和BT的三级版本 |
...
| footvolley | Bool | footvolley |
| mountaineering | Bool | 登山 |
| squat | Bool | 深蹲 |
| parachuting | Bool | 跳伞 |
| orienteering | Bool | 定向越野 |
| pickleball | Bool | pickleball |
...
| getSupportAppBaseInformation | Bool | 支持获取/下发 APP 基本信息 |
| supportGetAppBaseInformationVersion10 | Bool | APP 基本信息扩展字段（token、domain、app_key 等） |
| alarmBath | Bool | 洗澡 |
| alarmCourse | Bool | 课程 |
| alarmLearn | Bool | 学习 |
...
| supportSetSportRecordName | Bool | 支持设置运动记录名称（15.108，table71 BIT_6） |
| supportAppNotDisplaySportTypeSetting | Bool | app 不显示运动类型设置入口（table71 BIT_7） |
| supportPhoneCalendarReminder | Bool | 支持手机日历提醒（15.110，0x33/0x98，table71 BIT_4） |
